...
Un vettore $$ \overrightarrow{AB}$$ è composto da
...
$$\pi$$ è il luogo dei punti $$Q=(x,y,z)$$ dello spazio tali che
$$\overrightarrow{PQ}$$ è ortogonale a $$\underline{n}$$
($$\overrightarrow{PQ}\cdot\underline{n}=0$$).

<Details summary="Dati">

- $$P = (x_P,y_P,z_P)$$;
- $$\underline{n} = (a,b,c)$$;
- $$\overrightarrow{PQ} = (x-x_P,y-y_P,z-z_P)$$;
- $$\overrightarrow{PQ}\cdot\underline{n} = 0$$;
...
- $$\underline{w} = \overrightarrow{AC}$$;

</Details>

Un punto $$Q$$ sta su $$\pi$$ se e solo se $$ \overrightarrow{AQ}$$ può essere
descritto come somma di 2 vettori: uno con direzione uguale a $$\underline{v}$$
e uno con direzione uguale a $$\underline{w}$$:

$$\exists t,s\ \vert\  \overrightarrow{AQ} = t\underline{v}+s\underline{w}$$
...
- $$C = (x_C,y_C,z_C)$$;
- $$Q = (x,y,z)$$;
- $$\underline{v} = \overrightarrow{AB} = (v_1,v_2,v_3)$$;
- $$\underline{w} = \overrightarrow{AC} = (w_1,w_2,w_3)$$;
- $$\overrightarrow{AQ} = (x-x_A,y-y_A,z-z_A)$$
...
- $$\underline{v} = \overrightarrow{AB}$$ il vettore direzionale della retta
  $$r$$;
- $$t \in\mathbb{R}$$;
...
$$\overrightarrow{AQ}$$ ha la stessa direzione di $$\underline{v}$$ (quindi
$$\overrightarrow{AQ} = t\underline{v}$$)
