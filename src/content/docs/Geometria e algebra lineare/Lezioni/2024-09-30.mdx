...
conformabile a sinistra ad una matrice $$B \in \mu_{m' \times n'}(\mathbb{R})$$
...
$$C = [c_{ik}] = \mu_{m \times n'} \C c_{ik} = \SUM{j = 1}{n} a_{ij} b_{jk}$$
...
Date $$A_{m \times n} \C B_{n \times n'} \C C_{n' \times n''}$$:
