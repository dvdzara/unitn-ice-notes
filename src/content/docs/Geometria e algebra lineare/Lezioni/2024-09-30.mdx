...
$$\{ a_11, a_22, \ldots, a_mn \}$$ formano la diagonale principale della
matrice.
...
Notazione: $$\underline{0}_{m \times n} = [0_{ij}]$$.
...
Una matrice $$A \in \mu_{m \times n}(\mathbb{R}) = [a_{ij}]$$ è detta
conformabile a sinistra ad una matrice
$$B \in \mu_{m_1 \times n_1}(\mathbb{R})$$ se il numero di colonne di $$A$$ è
uguale al numero di righe di $$B$$, quindi $$n=n_1$$.
...
Il prodotto righe per colonne $$AB$$ è definito:11

$$C = [c_{ik}] = \mu_{m \times n_1}, \ c_{ik} = \sum_{j = 1}^{n} a_{ij} b_{jk}$$

La matrice identità è una matrice quadrata
$$A \in \mu_{m \times n}(\mathbb{R})$$ che ha coefficienti 1 sulla diagonale
principale o 0 altrimenti.

**Proprietà**:

Date $$A_{m \times n}, \ B_{n \times n_1}, \ C_{n_1 \times n_2}$$:
